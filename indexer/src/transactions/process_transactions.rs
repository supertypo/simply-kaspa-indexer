        ...
        if let Some(transaction_data) = txs_queue.pop() {
            ...
            for rpc_transaction in transaction_data.transactions {
                ...
                if tx_id_cache.contains_key(&transaction_id) {
                    ...
                    if enable_transactions_inputs_resolve && let Some(outputs) = &transaction.outputs {
            ...
            if block_tx.len() >= batch_size || (!block_tx.is_empty() && Instant::now().duration_since(last_commit_time).as_secs() > 2)
            {
                ...
                if !disable_address_transactions {
                    rows_affected_tx_addresses += if !exclude_tx_out_script_public_key_address {
                        insert_input_tx_addr(batch_scale, transaction_ids, database.clone()).await
                    } else if !exclude_tx_out_script_public_key {
                        insert_input_tx_script(batch_scale, transaction_ids, database.clone()).await
                    } else {
                        0
                    };
                }
...
async fn insert_input_tx_addr(batch_scale: f64, values: Vec<SqlHash>, database: KaspaDbClient) -> u64 {
    ...
    for batch_values in values.chunks(batch_size) {
        rows_affected += database
            .insert_address_transactions_from_inputs(batch_values)
            .await
...
async fn insert_input_tx_script(batch_scale: f64, values: Vec<SqlHash>, database: KaspaDbClient) -> u64 {
    let batch_size = min((100f64 * batch_scale) as u16, 8000) as usize;
    ...
    for batch_values in values.chunks(batch_size) {
        rows_affected +=
            database.insert_script_transactions_from_inputs(batch_values).await.unwrap_or_else(|e| panic!("Insert {key} FAILED: {e}"));
    }
    debug!("Committed {} {} in {}ms", rows_affected, key, Instant::now().duration_since(start_time).as_millis());
    rows_affected
}
...
fn resolve_inputs_from_concurrent_outputs(tx_outputs: HashMap<(SqlHash, i16), TransactionOutput>, transactions: &mut [Transaction]) {
    for tx in transactions.iter_mut() {
        if let Some(inputs) = &mut tx.inputs {
            for input in inputs.iter_mut() {
                let key = (input.previous_outpoint_hash.clone().unwrap(), input.previous_outpoint_index.unwrap());
                if let Some(tx_output) = tx_outputs.get(&key) {
                    input.previous_outpoint_script = tx_output.script_public_key.clone();
                    input.previous_outpoint_amount = tx_output.amount;
                }
            }
        }
    }
